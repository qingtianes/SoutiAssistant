...
| 智能匹配 | 统一 0～100 相关度；支持题干片段、乱序选项、正确答案正文反查 | 完成 |
...
- 支持 `.txt / .docx / .pdf / .xls`；`.xlsx` 请先另存为 `.xls`。
- 导入后会结构化保存题干、选项、答案，并提示重复题、空答案、异常选项数和疑似合并题。
- PDF 仅支持带文本层的文字版；扫描版 PDF、文档中的纯图片、图片公式/图形答案暂不能可靠解析。
- DOCX 的普通段落与表格文字可提取；复杂公式（OMML）、嵌入图片和特殊排版仍需导入后抽查。
- 搜题时三种入口使用同一匹配引擎，可按题干片段、乱序选项或长度足够的答案片段检索。
